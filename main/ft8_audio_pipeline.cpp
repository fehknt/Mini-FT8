...
void ft8_audio_pipeline_run(const ft8_audio_pipeline_config_t* cfg)
{
    ...
    // Outer loop: restarts whenever g_protocol_change_seq changes.
    // This reinits the monitor for the new protocol without tearing down
    // the USB connection (which would disconnect QMX CAT).
    while (!cfg->should_stop(cfg->ctx)) {

        // Snapshot current protocol at (re)start
        const ProtocolConfig* proto = (const ProtocolConfig*)g_protocol;
        uint32_t known_seq = g_protocol_change_seq;

        const int64_t slot_ms      = proto->slot_time_ms;
        const int     sym_delay_ms = (int)(proto->symbol_period * 1000.0f + 0.5f);
        const int     target_blocks  = proto->total_symbols + 1;
        const int     trigger_blocks = proto->total_symbols - 1;
        ...
        // Allocate at FT8 worst-case size so FT4 fits without realloc on switch
        const int buf_samples = (mon.block_size > 960) ? mon.block_size : 960;
        float* ft8_buffer = (float*)heap_caps_malloc(sizeof(float) * buf_samples, MALLOC_CAP_DEFAULT);
        ...
        // Sync to next slot boundary for this protocol
        ...
        ESP_LOGI(tag, "Pipeline started: proto=%s slot_ms=%lld sym_ms=%d trigger=%d",
                 proto->name, (long long)slot_ms, sym_delay_ms, trigger_blocks);

        // Inner loop: processes one sample at a time for this protocol
        bool protocol_changed = false;
        while (!cfg->should_stop(cfg->ctx) && !protocol_changed) {
            ...
            for (int i = 0; i < samples_dec && !cfg->should_stop(cfg->ctx) && !protocol_changed; i++) {
                ...
                if (ft8_buffer_idx >= mon.block_size) {
                    ...
                    if (now_idx != slot_idx) {
                        ...
                        // Check for protocol change at slot boundary (clean reinit point)
                        if (g_protocol_change_seq != known_seq) {
                            ESP_LOGI(tag, "Protocol change detected at slot boundary; reiniting");
                            protocol_changed = true;
                        }

                    } else if (slot_blocks >= trigger_blocks && mon.wf.num_blocks >= trigger_blocks) {
                        // Enough symbols collected — trigger decode
        ...
        free(temp_dec);
        monitor_free(&mon);
        // If protocol_changed: outer loop restarts with new g_protocol snapshot
        // If should_stop: outer loop exits
    }
}
