...
static void stream_uac_task(void* arg) {
    ...
    // Initialize monitor for current protocol
    ...
    // Allocate buffers. ft8_buffer is sized for the worst case (FT8 block_size).
    ...
    int target_blocks = g_protocol->total_symbols + 1;
    uint32_t last_proto_seq = g_protocol_change_seq;
        ...
        for (int i = 0; i < samples_dec && !s_stop_requested; i++) {
            ...
            if (ft8_buffer_idx >= mon.block_size) {
                ...
                if (now_idx != slot_idx) {
                    ...
                    // Apply pending protocol change here, while the monitor is at
                    // a clean boundary. Re-init monitor in place so USB stays up.
                    if (g_protocol_change_seq != last_proto_seq) {
                        ...
                        ft8_buffer_idx = 0;
                        // Re-derive slot index for the new slot timing.
                        now_idx = rtc_now_ms() / g_protocol->slot_time_ms;
                    }
                    ...
                    if (g_decode_enabled) {
                        ...
                        // Run decode on a separate task pinned to core 1 so that
                        // we wait via task notify instead of holding the stream
                        // task busy for the entire decode duration. Mirrors
                        // stream_mic_task's pattern.
                        ...
                        if (xTaskCreatePinnedToCore(decode_task, "decode_uac", 8192, p, 4, nullptr, 1) == pdPASS) {
                            ulTaskNotifyTake(pdTRUE, portMAX_DELAY);
                        } else {
                            ESP_LOGE(TAG, "decode task start failed; running inline");
                            delete p;
