...
void stream_mic_task(void* /*arg*/) {
  ...
  // Allocate chunk for FT8 worst-case block size; FT4 uses less.
  ...
  float* chunk = (float*)heap_caps_malloc(sizeof(float) * max_block_size, MALLOC_CAP_DEFAULT);
  if (!chunk) {
  ...
  while (true) {
    ...
    // Apply pending protocol change between slots.
    if (g_protocol_change_seq != last_proto_seq) {
      ESP_LOGI(TAG_MIC, "Applying protocol change -> %s", g_protocol->name);
      monitor_free(&mon);
      mon_cfg.protocol = g_protocol->protocol_id;
      mon_cfg.time_osr = g_time_osr;
      mon_cfg.freq_osr = g_freq_osr;
